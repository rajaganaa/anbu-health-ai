/**
 * App.jsx — Anbu Health AI (updated)
 *
 * Changes from original:
 *  1. Imports StructuredLabResult, StructuredMedicineResult, StructuredScanResult
 *     from ./StructuredResults
 *  2. callAnbuAPI now maps the real FastAPI response shape to what
 *     MessageBubble expects (msg.structured)
 *  3. MessageBubble dispatches on msg.structured.mode instead of
 *     checking for specific keys
 *  4. All other code is identical to your original
 */

import { useState, useRef, useEffect, useCallback } from "react";
import {
  StructuredLabResult,
  StructuredMedicineResult,
...
// ── Real API call — replaces the mock ────────────────────────────────────────
// Maps FastAPI /api/analyze response → { answer, structured, mode }
// structured shape matches buddhi.py's `sr` dict exactly.
async function callAnbuAPI(message, uploadedFile, mode) {
  ...
  // buddhi.structured_response has mode, urgency, confidence, summary,
  // findings, abnormal_findings, normal_findings (lab),
  // uses, side_effects, warnings, dosage (medicine),
  // body_part, scan_type, implants_detected, implant_details,
  // fractures_visible (scan), recommendation, disclaimer
  const structured = data.buddhi?.structured_response || null;

  return {
    mode:       data.mode,
    answer:     data.final_answer || data.sakshi?.final_answer || "",
    structured, // passed directly to msg.structured
...
// ── MessageBubble — dispatches to the right structured component ──────────────
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: "75%", minWidth: 80 }}>
        ...
        {msg.file && (
          <div style={{
            background: "rgba(255,255,255,0.06)", borderRadius: "12px 12px 0 0",
            ...
            <span style={{ fontSize: 18 }}>
              {msg.fileMode === "lab" ? "🧪" : msg.fileMode === "scan" ? "🩻" : "💊"}
            </span>
            <span>{msg.file}</span>
          </div>
        )}
        ...
        {!isUser && (
          <div style={{ display: "flex", gap: 8, marginTop: 5, paddingLeft: 4 }}>
            <button onClick={speakText} style={{
              ...
              fontSize: 11, color: audioPlaying ? "#10b981" : "rgba(255,255,255,0.35)",
...
// ── UploadModal (unchanged from your original) ────────────────────────────────
        ...
        {step === "success" ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              ...
              <input type="tel" maxLength={10} value={phone} onChange={e => setPhone(e.target.value.replace(/\D/g, ""))} placeholder="98765 43210"
          ...
          <>
            ...
            <input type="tel" maxLength={6} value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, ""))} placeholder="• • • • • •"
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 16px 12px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <button onClick={onNewChat} style={{ width: "100%", padding: "9px 12px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.7)", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontFamily: "inherit" }}>
        ...
        <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <div style={{ height: 4, background: "rgba(255,255,255,0.08)", borderRadius: 2, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${(promptCount / MAX_PROMPTS) * 100}%`, background: promptCount >= MAX_PROMPTS ? "#ef4444" : "linear-gradient(90deg, #059669, #10b981)", borderRadius: 2 }} />
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "8px" }}>
          ...
          {chats.map(chat => (
            <button key={chat.id} onClick={() => { onSelectChat(chat.id); onClose(); }}
              style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: "none", background: chat.id === activeChatId ? "rgba(16,185,129,0.12)" : "none", color: chat.id === activeChatId ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.5)", fontSize: 13, cursor: "pointer", textAlign: "left", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
              <HistoryIcon />
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, padding: "20px 20px 0", textAlign: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: 500 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {suggestions.map((s, i) => (
            <button key={i} onClick={() => onQuickPrompt(s.text)}
              style={{ padding: "11px 12px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.03)", color: "rgba(255,255,255,0.6)", fontSize: 12, lineHeight: 1.5, cursor: "pointer", textAlign: "left", fontFamily: "inherit", display: "flex", alignItems: "flex-start", gap: 8 }}>
...
export default function AnbuHealthAI() {
  ...
  const handleSend = async (textOverride) => {
    ...
    const msgText = text || `${pendingFile?.name} analyze பண்ணு`;
    const userMsg = {
    ...
    try {
      ...
      addMessage(activeChatId, {
        ...
        structured: result.structured,  // has .mode for dispatch
        timestamp: Date.now(),
      });
    } catch (e) {
      addMessage(activeChatId, {
        id: Date.now() + 1, role: "assistant",
        content: "Sorry, error ஆச்சு. மீண்டும் try பண்ணுங்க.",
        timestamp: Date.now(),
      });
    } finally {
      setIsLoading(false);
  ...
  const handleUpload = (file, mode) => {
    ...
    setShowUploadModal(false); inputRef.current?.focus();
  };
  ...
  return (
    <div style={{ height: "100vh", background: "#0f1117", display: "flex", flexDirection: "column", fontFamily: "'Segoe UI', system-ui, sans-serif", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.07)", background: "#0f1117", flexShrink: 0 }}>
        <button onClick={() => setSidebarOpen(true)} style={{ background: "none", border: "none", cursor: "pointer", padding: 8, borderRadius: 8, color: "rgba(255,255,255,0.6)" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 8px" }}>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          {messages.length === 0 ? (
            ...
            <>
              {messages.map((msg, i) => <MessageBubble key={msg.id} msg={msg} isLast={i === messages.length - 1} />)}
      ...
      {promptCount >= MAX_PROMPTS && (
        <div style={{ padding: "8px 16px", maxWidth: 696, margin: "0 auto", width: "100%" }}>
          <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 10, padding: "8px 14px", fontSize: 12, color: "#fca5a5", textAlign: "center" }}>
            ⚠️ Today's 20 prompts மொத்தமும் use ஆயிட்டு. நாளைக்கு வா!
          </div>
        </div>
      )}
      ...
      <div style={{ padding: "8px 16px 16px", flexShrink: 0, maxWidth: 696, margin: "0 auto", width: "100%" }}>
        <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, display: "flex", alignItems: "flex-end", gap: 4, padding: "10px 12px" }}>
              ...
              <div style={{ position: "absolute", bottom: "calc(100% + 8px)", left: 0, background: "#1a1f2e", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: 8, minWidth: 220, zIndex: 30, animation: "slideUp 0.2s ease", boxShadow: "0 16px 48px rgba(0,0,0,0.5)" }}>
                {plusMenuItems.map(item => (
                  <button key={item.mode} onClick={() => { setUploadMode(item.mode); setShowUploadModal(true); setShowPlusMenu(false); }}
                    style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "none", background: "none", color: "white", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontFamily: "inherit" }}
          ...
          <button onClick={handleVoice}
            style={{ width: 36, height: 36, borderRadius: 10, border: "none", background: isListening ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.07)", color: isListening ? "#ef4444" : "rgba(255,255,255,0.5)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <MicIcon active={isListening} />
          </button>

          <button onClick={() => handleSend()}
            disabled={(!inputText.trim() && !pendingFile) || isLoading || promptCount >= MAX_PROMPTS}
            style={{ width: 36, height: 36, borderRadius: 10, border: "none", background: (inputText.trim() || pendingFile) && !isLoading && promptCount < MAX_PROMPTS ? "linear-gradient(135deg, #059669, #10b981)" : "rgba(255,255,255,0.07)", color: (inputText.trim() || pendingFile) && !isLoading && promptCount < MAX_PROMPTS ? "white" : "rgba(255,255,255,0.25)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <SendIcon />
          </button>
        </div>
